...
const Pest_Categories = ({ blogList }) => {
  const [count, setCount] = useState(0);
  const [toggleBlog, setToggleBlog] = useState(false);
  const [findBlog, setFindBlog] = useState("");

  const Pest_category = blogList?.filter(({ category }) => category === "Pest");
  ...
  const SearchBlog = Pest_category?.filter(
    (p) =>
...
export default Pest_Categories;
